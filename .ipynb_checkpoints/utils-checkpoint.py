...
def import_and_clean_data():
    ...
    df_bp = ( # bp = Best Party (Preferences)
        pd.read_csv('best_party.csv', sep='\t', encoding='latin') # Tab-separated with header from SCB
        .reset_index()   
    )
    ...
    pd.read_csv('next_best_party.csv', sep='\t', encoding='latin') # Tab-separated with header from SCB
    .reset_index()
    )
